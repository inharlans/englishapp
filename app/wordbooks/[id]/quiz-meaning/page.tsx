...
export default async function WordbookQuizMeaningPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (!user) {
    return (
      <section className="space-y-4">
        ...
        <Link href={{ pathname: "/login", query: { next: "/wordbooks" } }} className="text-sm font-semibold text-blue-700 hover:underline">
          로그인
        </Link>
      </section>
    );
  ...
  return <WordbookQuizClient wordbookId={id} initialMode="MEANING" />;
}
