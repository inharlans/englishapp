...
import { WordbookStudyClient } from "./studyClient";
...
export default async function WordbookStudyPage(props: { params: Promise<{ id: string }> }) {
  const user = await getUserFromRequestCookies(await cookies());
  if (!user) {
    return (
      <section className="space-y-4">
        <h1 className="text-2xl font-black tracking-tight text-slate-900">Wordbook Study</h1>
  ...
  return <WordbookStudyClient wordbookId={id} />;
}
