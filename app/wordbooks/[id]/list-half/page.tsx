...
export default async function WordbookListHalfPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (!user) {
    return (
      <section className="space-y-4">
        ...
        <Link href={{ pathname: "/login", query: { next: `/wordbooks/${id}/list-half` } }} className="text-sm font-semibold text-blue-700 hover:underline">
          로그인
        </Link>
      </section>
    );
  ...
  return <WordbookListClient wordbookId={id} mode="listHalf" title="회복 목록" />;
}
