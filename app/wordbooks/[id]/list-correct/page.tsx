...
export default async function WordbookListCorrectPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (!user) {
    return (
      <section className="space-y-4">
        ...
        <Link href={{ pathname: "/login", query: { next: `/wordbooks/${id}/list-correct` } }} className="text-sm font-semibold text-blue-700 hover:underline">
          로그인
        </Link>
      </section>
    );
  ...
  return <WordbookListClient wordbookId={id} mode="listCorrect" title="정답 목록" />;
}
