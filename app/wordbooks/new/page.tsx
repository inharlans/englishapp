...
export default function NewWordbookPage() {
  ...
  const validations = useMemo<RowValidation[]>(() => {
    ...
    return rows.map((row, index) => {
      ...
      const issues: string[] = [];
      if (!term) issues.push("term 필요");
      if (!meaning) issues.push("meaning 필요");
  ...
  const parsePaste = () => {
    ...
    setStatus(`불러온 행: ${parsed.length}개`);
  };
      ...
      if (!saveRes.ok) {
        ...
        if (typeof window !== "undefined") {
          window.localStorage.setItem(
            `pending_wordbook_items_${wordbookId}`,
        ...
        setStatus(saveJson.error ?? "단어장은 생성되었지만 단어 저장에 실패했습니다.");
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <pre className="mt-2 overflow-x-auto text-xs text-slate-700">
              <code>{`index<TAB>en<TAB>ko
1<TAB>mister<TAB>(명)-님, -씨, 미스터
2<TAB>vacation<TAB>(명)휴가
3<TAB>client<TAB>(명)객, 고객, 손님, 고객님, 클라이언트`}</code>
            </pre>
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
...
1,mister,"(명)-님, -씨, 미스터"
2,vacation,"(명)휴가"
3,client,"(명)객, 고객, 손님, 고객님, 클라이언트"`}</code>
            </pre>
          </div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            type="button"
            onClick={() => void copyText(tsvExample, "TSV 예시를 복사했습니다.")}
          ...
          <button
            type="button"
            onClick={() => void copyText(csvExample, "CSV 예시를 복사했습니다.")}
            className="ui-btn-secondary px-3 py-1.5 text-xs"
          >
            CSV 예시 복사
          </button>
            ...
            <div className="mt-3 space-y-3">
              <label className="block">
                ...
                <textarea
                  ...
                  rows={8}
                  className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                  placeholder="index,en,ko 또는 en,ko"
                />
              </label>
          ...
          {tab === "manual" ? (
            <div className="mt-3 space-y-3">
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => addRows(10)} className="ui-btn-secondary px-3 py-1.5 text-xs">
                  +10 rows
                </button>
              </div>
                ...
                <table className="w-full min-w-[520px] text-sm">
                  <thead className="bg-slate-50 text-left text-xs text-slate-600">
                    <tr>
                      ...
                      <th className="px-3 py-2">term (영어)</th>
                      <th className="px-3 py-2">meaning (한국어)</th>
          ...
          <section className="ui-card p-5">
            <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
              ...
              <button
                ...
                className="ml-auto ui-btn-secondary px-2 py-1 text-xs"
              >
