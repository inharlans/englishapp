...
### 2026-02-22 Iteration 64
- 퀴즈 초기 로딩 구간의 상단 요약 수치(`총 개수/파트/남은 문제`)를 `-` 표기로 보정
...
### 2026-02-22 Iteration 68
- `quiz-word` 실플레이 기준으로 피드백(정답/오답) 노출 중 정답 입력창을 비활성화
...
### 2026-02-22 Iteration 81
- `/wordbooks/[id]` 상세 상단 `뒤로` 링크를 사용자 맥락 기반으로 보정
...
### 2026-02-22 Iteration 87
- `/wordbooks/market` 게스트 안내문의 인라인 로그인 링크를 `next` 보존 방식으로 통일
...
### 2026-02-22 Iteration 91
- `/offline/wordbooks/[id]` 학습 화면에서도 빈 성공 상태 박스가 상시 노출되던 문제 수정
...
### 2026-02-22 Iteration 96
- `wordbooks/[id]` 퀴즈 화면의 빈 상태 메시지(`NBSP`) 상시 노출 패턴 제거
