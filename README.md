...
## 2026-02-17 Preview bypass for GPT/session testing

- Added temporary middleware bypass endpoint: `/preview-access?token=...&next=/path`
- Environment variable: `PREVIEW_BYPASS_TOKEN`
- On valid token, server sets `preview_bypass` httpOnly cookie (1 hour, `SameSite=Lax`).
- While cookie exists, auth redirect/middleware protection is bypassed for preview/testing.

Usage:
1. Set `PREVIEW_BYPASS_TOKEN` in server env.
2. Open `/preview-access?token=<TOKEN>&next=/` once.
3. Continue browsing protected pages in the same session.
4. Remove token env (or wait cookie expiry) to disable.
