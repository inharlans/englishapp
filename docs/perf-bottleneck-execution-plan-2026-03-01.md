...
- [x] 기존 normalized 조회를 chunk 반복에서 단일 조회로 변경

### 5) Market 개선
- [x] count/list 이중 쿼리를 단일 집계/페이지 쿼리로 정리
- [x] `LATERAL` 반복 count 의존 경로 제거

### 6) 관측 오버헤드 완화
- [x] `lib/api/metric-response.ts`에서 메트릭 저장을 `await`에서 비동기(`void ...catch`)로 전환
