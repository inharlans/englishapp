...
export function PostDownloadOnboardingBanner({
  ...
  return (
    <aside
      className="rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-50 to-blue-50 p-4 shadow-sm"
      role="status"
      aria-live="polite"
      aria-atomic="true"
    >
      ...
      <h2 className="mt-1 text-lg font-black text-slate-900">
        {pending.title?.trim() ? `"${pending.title.trim()}"` : "다운로드 단어장"} 학습을 시작할까요?
      </h2>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button
          type="button"
          onClick={dismiss}
          className="rounded-lg border border-transparent px-3 py-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700"
          aria-label="오늘은 배너 닫기"
        >
          오늘은 닫기
        </button>
      </div>
